...
顶部规则：
- 面包屑、round id、requirement 单行摘要、status/outcome badge、主要操作与 trigger/repairLoopsUsed/currentNode stats 同属统一 Page Header。
- status/outcome badge 与 round id 在同一标题组内；requirement 摘要位于标题下方，stats 位于标题与操作下方，避免挤压 run/trigger 文案。
...
- requirement 摘要与“查看完整需求”入口放在标题下方。
- round status/outcome 放在标题组，trigger、repairLoopsUsed、currentNode 放入顶部 stats。
