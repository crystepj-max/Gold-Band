...
- 2026-05-12 验收修正：ACP 会话抽屉禁止因 Raw frames 切换、raw frame 展开或 tool call 展开自动滑到底部；pending/running 空事件态展示“Claude 调起中”，运行过程中持续展示处理中动效、当前步骤计时和总耗时；thought/tool/plan/处理中状态不展示头像但保留工具卡横向位置，工具卡高度更紧凑；工具标题左对齐显示“操作名 + 次级参数”。
