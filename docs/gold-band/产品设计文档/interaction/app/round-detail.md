...
会话区包含：
- Session header：压缩为两行，只展示 provider/adapter 显示名、国际化状态、原始帧小按钮和 provider session id；provider session id 从当前 attempt 的 `worker-ref.json` 派生，`acp.session.json` 只作为运行态快照；不展示 cwd、恢复标记、事件数或错误数。
...
- 页面布局对齐桌面工作台：顶部使用统一 Page Header 承载面包屑、round id、requirement 摘要、status/outcome、trigger、repairLoopsUsed、当前节点/结束节点与直接操作；终态 round 使用“结束节点”文案，避免暗示节点仍在运行；主体默认展示实际工作图，详情以右侧 Sheet 抽屉按需展示。
