...
- Message list：基于 prompt-kit `ChatContainer` / `Message` 展示 agent/user 文本气泡；初始 prompt 和后续继续输入都作为右侧用户消息出现；新建 ACP session 时 system prompt 通过 `_meta.systemPrompt.append` 注入，继续输入发送到 ACP 时只包含用户 prompt，不追加内部续聊说明，也不展示 system prompt。
...
- Permission：permission request 使用轻量 inline action bar，而不是大块表单卡片；左侧展示权限图标、工具/权限标题和 pending 状态，右侧用紧凑操作按钮承载允许/拒绝选项；用户点击允许或拒绝后立即退出 pending UI，失败时恢复并提示。
- Composer：基于 prompt-kit `PromptInput` 用于继续 ACP 会话；点击发送后立即清空并乐观展示右侧用户气泡，permission pending 时禁用普通输入；输入区下方以只读信息条展示当前 adapter 生效的模型与权限模式，不提供修改入口。
...
- Scroll：会话在发送用户 prompt、轮询获得新 ACP event 或 agent 回复追加内容时显式贴底，处理中提示结束时不能因 session 刷新保留旧 scrollTop 而产生先跳顶部再回底部的视觉抖动；切换 Raw frames、展开 raw frame 或展开 tool call 只保留当前阅读位置，不触发自动滑到底部。
- Processing：pending/running 且尚无可展示事件时在 composer 内显示“Claude 调起中”；用户点击发送后、`session/prompt` 请求尚未完成时显示“发送中”动效；请求完成且消息已发出后到首帧之间切换为“处理中”动效，右侧乐观用户气泡下方也从“发送中...”切到“处理中...”，但总耗时不计入发送等待阶段；首帧后按最新事件类型展示“思考中 / 工具调用中 / 回复生成中”；当前步骤耗时只在处理中展示，总耗时按每轮“首个非用户响应帧到该轮响应结束”累加并在 composer 内常驻展示，不把两轮之间的用户空闲时间计入总耗时；继续 ACP session 时 `session/load` 回放的历史消息不重复追加到 UI 事件流，已有聊天历史仍按原顺序显示；消息流不插入独立处理中卡片。
...
- status/outcome 只来自 canonical state；ACP session events、日志和 raw frame 仅作为会话观测内容；ACP session status 不作为节点主状态展示，运行态轮询只依据结构化 run/round/node 状态，不扫描会话详情文本或历史 events。
