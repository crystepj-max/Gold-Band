...
- 2026-05-07 起实际工作图打开后必须在画布可视区域内默认完整居中展示；GraphView 使用受控 viewport 按节点 bounds 和容器尺寸计算初始平移/缩放，且实际工作图容器不得设置超过父内容区的固定最小高度，避免执行路径图底部圆角和节点卡片被父级 `overflow-hidden` 裁切。
