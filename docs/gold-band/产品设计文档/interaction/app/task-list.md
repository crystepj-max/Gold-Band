...
- Sheet header 固定在滚动区外，requirement、执行统计和操作按钮位于内部 ScrollArea。
- quote、requirement summary、run id、workflow 状态和按钮文案都必须在抽屉内换行或截断，不允许撑破容器。
...
- Task Preview Sheet 展示 task id、状态、requirement 摘要、latest run、artifact/attachment 统计和进入/产物操作入口。
