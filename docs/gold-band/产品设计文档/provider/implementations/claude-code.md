...
> Claude Code implementation 是 Gold Band provider 体系中的默认 B()：它不负责重新组织 runtime 输入，而是负责消费 runtime 已准备好的 `prompt bundle`，把它稳定映射到 Claude Code 的会话、输入、结果提取与观测能力上；其中 `outputArtifact.content` 只保留模型返回的 raw content。