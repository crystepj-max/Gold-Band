...
- “使用本地 Claude”只影响 Claude ACP adapter 的 `CLAUDE_CODE_EXECUTABLE` 注入。Windows 上需要同时兼容原生安装和 npm 安装：原生安装优先使用 PATH 中的 `claude.exe`；npm 安装若只暴露 `claude` / `claude.cmd` shim，则应反查同目录 `node_modules/@anthropic-ai/claude-code/bin/claude.exe` 并注入该原生 binary，不能把 extensionless `claude` shell shim 传给 adapter；若找不到原生 binary，则不注入环境变量。macOS / Linux 继续按 PATH 查找 `claude`。
