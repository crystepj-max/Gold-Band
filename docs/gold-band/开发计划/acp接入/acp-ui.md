...
权限请求必须保留用户选择、时间和相关 tool call id，便于后续排障。用户点击允许或拒绝后，UI 立即乐观关闭 pending 卡片；若响应失败，再恢复卡片并提示重试。pending / waiting 状态使用低强调 primary 语义色，不使用 warning 橙色；按钮使用紧凑胶囊形态，长选项文本单行截断。
...
新增用户 prompt、轮询获得新 ACP event 或 agent 回复追加内容时，ChatContainer 必须贴底；处理中提示结束时只移除 composer/乐观气泡状态，不允许 session 刷新导致消息区先跳顶部再回底部。

### 6.9 处理中反馈与计时

- 会话处于 pending / running 且尚无可渲染事件时，composer 内显示“Claude 调起中”，Message List 不显示“暂无 ACP 事件”。
- 用户点击发送后、`session/prompt` 请求尚未完成时，在底部 composer 输入区和右侧乐观用户气泡下方显示“发送中...”；请求完成且消息已发出后到首个非用户帧返回前，切换为“处理中...”并展示当前已完成总耗时，但发送等待阶段本身不计入总耗时，避免长时间无变化被误判为卡死；composer action 行与发送按钮保留足够间距，避免按钮贴近输入框。
- 首帧返回后，composer 状态根据最新可见事件类型切换为“思考中 / 工具调用中 / 回复生成中”；会话进入 completed / failed / cancelled 后停止当前步骤计时，但总耗时按每轮“首个非用户响应帧到该轮响应结束”的耗时累加，并继续在 composer 中常驻展示；两轮之间的用户空闲时间不计入总耗时。
...
UI 上应避免把 ACP `stopReason`、session status 或 tool call status 直接展示成 Gold Band node status/outcome；ACP 会话头部不展示 session status，处理中状态由 composer 表达。Gold Band runtime canonical state 仍由 task / run / round / node / attempt / artifact 维护。
