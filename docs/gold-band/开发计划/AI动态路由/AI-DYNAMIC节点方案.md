...
- AI-DYNAMIC 节点作者态权限字段的编辑与回显统一使用 `permission_mode`。前端回显层需要兼容历史持久化数据中的 `permissionMode`，而 Rust DSL 的序列化/反序列化也必须兼容两种键名但统一持久化输出 `permission_mode`，否则用户保存自定义工作流后重新打开 Inspector 会看到权限模式被错误回退到”默认 / 不设置”。当前编辑器中的权限模式是整个 AI-DYNAMIC 内部派生节点统一采用的默认设置，不额外暴露内部节点逐个覆写入口；该字段允许保存 provider doctor 返回的真实 ACP mode id，例如 Claude 的 `bypassPermissions`，不能被 DSL 静态校验误判为非法虚拟值。
...
上面的 JSON 只用于解释语义。真实输出契约以 runtime 生成的有效 JSON Schema 为准：

- Rust `DynamicNodeCompletion` / `DynamicNext` / `DynamicNodeSpec` / `DynamicAgentTaskSpec` 数据结构通过 `schemars` 生成基础 schema。
- runtime 根据当前 AI-DYNAMIC 配置收窄 schema：fixed/dynamic Agent 策略决定 provider 是否 required 或 forbidden，模型策略决定 model 是否 required 或 forbidden，可用 worker profile 与 allowed workflow snapshot 会变成枚举，`maxFanout` 会变成 `next.nodes.maxItems`。
- 同一份有效 schema 会注入 provider output contract，嵌入 `src/prompts/<lang>/runtime/ai-dynamic/output_protocol.md`，并由 runtime 使用 `jsonschema` 在 serde 反序列化前做结构校验。
- schema 只负责结构与当前运行策略约束；id 去重、dependsOn、环检测、group depth、workflow snapshot 合法性等图语义仍由 Rust 业务校验聚合。
...
proposal 的 rejected 结果使用结构化错误对象而不是裸字符串，至少包含 `code`、`message`、`params`，并可携带 `path / actual / expected / allowedValues / suggestion`。

校验分两层：

- 有效 JSON Schema 先校验结构与当前运行策略，覆盖 required、additional properties、enum、type、fanout 数量、fixed 策略 forbidden provider/model、merge/acceptance forbidden profile 等错误，并把 `jsonschema` 诊断转换为统一的 proposal validation error。
- Rust 语义校验继续负责跨节点和跨图规则，覆盖 id 去重、依赖、环、depth、allowed workflow snapshot、profile/provider 实际解析、预算和幂等。
