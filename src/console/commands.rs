...
pub fn parse_console_command(input: &str) -> Result<ParsedConsoleCommand> {
    ...
    match parts.as_slice() {
        ...
        ["/run", "status", task_id, run_id] => Ok(ParsedConsoleCommand::Runtime(Command::Run(RunCommand::Status {
            task_id: (*task_id).to_string(),
            run_id: (*run_id).to_string(),
        }))),
        ["/run", "continue", task_id, run_id] => Ok(ParsedConsoleCommand::Runtime(Command::Run(RunCommand::Continue {
            task_id: (*task_id).to_string(),
            run_id: (*run_id).to_string(),
        }))),
        ["/run", "retry", task_id, run_id] => Ok(ParsedConsoleCommand::Runtime(Command::Run(RunCommand::Retry {
            task_id: (*task_id).to_string(),
            run_id: (*run_id).to_string(),
        }))),
        ["/run", "kill", task_id, run_id] => Ok(ParsedConsoleCommand::Runtime(Command::Run(RunCommand::Kill {
            task_id: (*task_id).to_string(),
            run_id: (*run_id).to_string(),
        }))),
        ["/run", "open-session", task_id, run_id, "--round", round, "--node", node, "--attempt", attempt] => {
            Ok(ParsedConsoleCommand::Runtime(Command::Run(RunCommand::OpenSession {
                task_id: (*task_id).to_string(),
                run_id: (*run_id).to_string(),
        ...
        ["/artifact", "list", task_id, run_id, "--round", round, "--node", node, "--attempt", attempt] => {
            Ok(ParsedConsoleCommand::Runtime(Command::Artifact(ArtifactCommand::List {
                task_id: (*task_id).to_string(),
                run_id: (*run_id).to_string(),
                round: (*round).to_string(),
                node: (*node).to_string(),
        ...
        ["/artifact", "show", task_id, run_id, "--round", round, "--node", node, "--attempt", attempt, "--name", name] => {
            Ok(ParsedConsoleCommand::Runtime(Command::Artifact(ArtifactCommand::Show {
                task_id: (*task_id).to_string(),
                run_id: (*run_id).to_string(),
                round: (*round).to_string(),
                node: (*node).to_string(),
...
pub fn suggest_console_commands(input: &str) -> Vec<String> {
    ...
    let trimmed = input.trim();
    if trimmed.is_empty() {
        return TOP_LEVEL_COMMANDS.iter().map(|item| (*item).to_string()).collect();
    }
    if !trimmed.starts_with('/') {
        return Vec::new();
    }
