...
use crate::dsl::{
    AiDynamicNode, DynamicAgentConfigDsl, NodeDsl, ValidatedWorkflow, WorkflowDsl,
...
const AI_DYNAMIC_FANOUT_PROMPT: &str = include_str!("../prompts/ai_dynamic_fanout.md");
const AI_DYNAMIC_MERGE_PROMPT: &str = include_str!("../prompts/ai_dynamic_merge.md");
const AI_DYNAMIC_ACCEPTANCE_PROMPT: &str = include_str!("../prompts/ai_dynamic_acceptance.md");
const AI_DYNAMIC_NODE_TASK_PROMPT: &str = include_str!("../prompts/ai_dynamic_node_task.md");
...
fn load_or_create_dynamic_graph(ctx: &DynamicExecutionContext<'_>) -> Result<DynamicGraphState> {
    ...
    let bootstrap = DynamicNodeState {
        ...
        task: AI_DYNAMIC_FANOUT_PROMPT.trim().to_string(),
        ...
        provider: ctx.dynamic.provider.clone(),
        profile: None,
        workflow_id: None,
...
fn drive_dynamic_graph(
    ...
    loop {
        ...
        if let Some(index) = graph
            .nodes
            .iter()
            .position(|node| node.status == DynamicNodeStatus::Ready)
        {
            execute_dynamic_node_at(ctx, graph, index)?;
            continue;
        }
...
fn execute_dynamic_node_at(
    ...
    match kind {
        DynamicNodeKind::Worker => execute_dynamic_worker(ctx, graph, index),
        DynamicNodeKind::WorkflowInvocation => {
            execute_dynamic_workflow_invocation(ctx, graph, index)
        }
        DynamicNodeKind::Merge | DynamicNodeKind::Acceptance => {
            execute_dynamic_agent_stage(ctx, graph, index)
        }
    }
}
...
fn execute_dynamic_worker(
    ...
    ensure_dynamic_workspace(ctx, &mut graph.nodes[index])?;
    ...
    finalize_dynamic_worker_result(ctx, graph, index, &attempt_id, result)?;
    if graph.nodes[index].outcome != Some(NodeOutcome::Success) {
        pause_dynamic_graph(ctx, graph, "dynamic worker failed")?;
        bail!("dynamic worker `{}` failed", graph.nodes[index].id);
    }
    accept_dynamic_completion_from_artifact(ctx, graph, index, &attempt_id)
}
...
fn execute_dynamic_agent_stage(
    ...
    graph.nodes[index].workspace_path = Some(ctx.app.paths.repo_root.clone());
    let node = graph.nodes[index].clone();
    let attempt_id = dynamic_attempt_id(&node);
    prepare_dynamic_attempt_dirs(ctx, &node, &attempt_id)?;
    let invocation = build_dynamic_worker_invocation(ctx, graph, &node, &attempt_id, None)?;
    let provider_id = node
        .provider
        .as_deref()
    ...
    finalize_dynamic_worker_result(ctx, graph, index, &attempt_id, result)?;
    if graph.nodes[index].outcome != Some(NodeOutcome::Success) {
        pause_dynamic_graph(ctx, graph, "dynamic merge or acceptance failed")?;
        bail!("dynamic stage `{}` failed", graph.nodes[index].id);
    }
    append_dynamic_event(
        ctx,
        ...
        serde_json::json!({
            ...
            "outcome": graph.nodes[index].outcome,
        }),
    )?;
    Ok(())
}
...
fn execute_dynamic_workflow_invocation(
    ...
    prepare_dynamic_attempt_dirs(ctx, &node, &attempt_id)?;
    let child_workflow =
        workflow_with_dynamic_invocation_task(snapshot.workflow.clone(), &node.task);
        ...
        .run_start(ctx.task_id, Some(child_workflow_path.as_path()))?;
    {
        let current = graph
            ...
            .get_mut(index)
            .ok_or_else(|| anyhow!("dynamic node index out of range"))?;
        current.child_run_id = Some(child_run.id.clone());
        current.finished_at = Some(now_rfc3339_like());
        current.status = DynamicNodeStatus::Completed;
        current.outcome = Some(if child_run.outcome == Some(RunOutcome::Success) {
            NodeOutcome::Success
        } else {
            NodeOutcome::Failure
        });
    ...
    if graph.nodes[index].outcome != Some(NodeOutcome::Success) {
        pause_dynamic_graph(ctx, graph, "child workflow failed")?;
        bail!(
    ...
    accept_dynamic_completion(ctx, graph, index, completion, None, None)
}

fn workflow_with_dynamic_invocation_task(mut workflow: WorkflowDsl, task: &str) -> WorkflowDsl {
    for node in &mut workflow.nodes {
        if let NodeDsl::Worker(worker) = node {
            worker.goal = Some(match worker.goal.as_deref() {
                Some(goal) if !goal.trim().is_empty() => format!(
                    "Child workflow invocation task:\n{}\n\nNode goal:\n{}",
                    task.trim(),
                    goal.trim()
                ),
...
fn accept_dynamic_completion_from_artifact(
    ctx: &DynamicExecutionContext<'_>,
    graph: &mut DynamicGraphState,
    index: usize,
    attempt_id: &str,
    ...
    let artifact_path = ctx.app.paths.dynamic_node_artifact_file(
        ...
        "dynamic node `{node_id}` did not produce dynamic-node-completion"
    );
...
fn accept_dynamic_completion(
    ...
    let source_node_id = graph.nodes[index].id.clone();
    let proposal_id = format!("proposal-{}-001", safe_dynamic_ref(&source_node_id));
    let proposal_artifact_path = artifact_path.unwrap_or_else(|| {
        ctx.app
            .paths
            .dynamic_dir(
                ctx.task_id,
    ...
    if let Err(err) = validate_dynamic_completion(graph, index, &completion).and_then(|_| {
        ensure!(
        ...
        pause_dynamic_graph(ctx, graph, "invalid dynamic-node-completion proposal")?;
        return Err(err);
    }
    let proposal = DynamicProposalState {
        version: VERSION.to_string(),
        id: proposal_id.clone(),
        dynamic_run_id: graph.run.id.clone(),
        source_node_id: source_node_id.clone(),
    ...
    materialize_dynamic_next(ctx, graph, index, completion.next)?;
    append_dynamic_event(
        ctx,
    ...
    match next {
        ...
        DynamicNext::Fanout {
            ...
            let group_depth = source
                ...
                .unwrap_or(1);
            let merge =
                dynamic_group_agent_spec(&ctx.dynamic.merge, merge.title, AI_DYNAMIC_MERGE_PROMPT);
            let acceptance = dynamic_group_agent_spec(
                &ctx.dynamic.acceptance,
                acceptance.title,
                AI_DYNAMIC_ACCEPTANCE_PROMPT,
            );
...
fn refresh_dynamic_ready_nodes(graph: &mut DynamicGraphState) {
    ...
    let running_count = graph
        .nodes
        .iter()
        .filter(|node| node.status == DynamicNodeStatus::Running)
        .count();
    let mut available_slots =
        (graph.run.control.max_parallel as usize).saturating_sub(running_count);
...
fn build_dynamic_worker_invocation(
    ...
    let builtin_profile = dynamic_builtin_profile(node);
    ...
    let profile_content = match builtin_profile {
        ...
        None => node
            .profile
            .as_deref()
            .map(|profile| ctx.app.profile_show(profile).map(|entry| entry.content))
            .transpose()?,
    };
    ...
    Ok(WorkerInvocation {
        ...
        requirement_text: Some(dynamic_requirement_text(ctx, graph, node)?),
        workspace_dir,
        attempt_dir: runtime_context.attempt_dir.clone(),
        output_contract,
        runtime_context,
        predecessors: dynamic_predecessor_contexts(ctx, graph, node),
        task_instruction: Some(dynamic_task_instruction(graph, node)),
        session_mode: SessionMode::New,
...
fn dynamic_builtin_profile(node: &DynamicNodeState) -> Option<(&'static str, &'static str)> {
    match node.kind {
        DynamicNodeKind::Worker if node.depth == 0 && node.chain_id == "bootstrap" => {
            Some(("ai-dynamic-fanout", AI_DYNAMIC_FANOUT_PROMPT))
        }
        DynamicNodeKind::Merge => Some(("ai-dynamic-merge", AI_DYNAMIC_MERGE_PROMPT)),
        DynamicNodeKind::Acceptance => {
            Some(("ai-dynamic-acceptance", AI_DYNAMIC_ACCEPTANCE_PROMPT))
        }
        _ => None,
    }
}

fn dynamic_requirement_text(
    ctx: &DynamicExecutionContext<'_>,
    graph: &DynamicGraphState,
    node: &DynamicNodeState,
) -> Result<String> {
    ...
    Ok(format!(
        ...
        base_requirement.trim(),
        graph.run.id,
        node.id,
        node.kind,
        node.group_id.as_deref().unwrap_or("none"),
        node.chain_id,
        node.depth,
        node.workspace.mode,
        node.workspace_path
        ...
        serde_json::to_string_pretty(&graph.run.control)?,
    ))
}

fn dynamic_task_instruction(graph: &DynamicGraphState, node: &DynamicNodeState) -> String {
    let completed_nodes = graph
        .nodes
        .iter()
        .filter(|item| item.status == DynamicNodeStatus::Completed)
        .map(|item| format!("{}({:?})", item.id, item.outcome))
        .collect::<Vec<_>>()
        .join(", ");
    let metadata = AI_DYNAMIC_NODE_TASK_PROMPT
        ...
        .replace("{completed_nodes}", &completed_nodes);
