...
fn main() {
    ...
    println!(
        "cargo:rustc-env=GOLD_BAND_RELEASE_CHANNEL={}",
        config.channel
    );
    println!("cargo:rustc-env=GOLD_BAND_APP_NAME={}", config.app_name);
    println!("cargo:rustc-env=GOLD_BAND_APP_KEY={}", config.app_key);
    println!(
        "cargo:rustc-env=GOLD_BAND_CONFIG_DIR_NAME={}",
        config.config_dir_name
    );
    println!(
        "cargo:rustc-env=GOLD_BAND_HOME_ENV_VAR={}",
        config.home_env_var
    );
    println!(
        "cargo:rustc-env=GOLD_BAND_UPDATER_ENDPOINT={}",
        config.updater_endpoint
    );
    println!(
        "cargo:rustc-env=GOLD_BAND_UPDATER_PUBLIC_KEY={}",
