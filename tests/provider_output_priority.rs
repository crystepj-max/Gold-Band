...
fn prefers_progress_events_over_raw_stream() {
    ...
    std::fs::write(attempt_dir.join("progress.events.jsonl").as_std_path(), "progress-event").unwrap();
    std::fs::write(attempt_dir.join("raw.stream.jsonl").as_std_path(), "raw-stream").unwrap();
